...
function FeasibilityBadge({ rating }) {
  if (rating === "safe")
    return (
      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-green-100 text-green-800 text-sm font-bold">
        🟢 Safe Purchase
      </span>
    );
  if (rating === "warning")
    return (
      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 text-sm font-bold">
        🟡 Proceed with Caution
      </span>
    );
  if (rating === "unsafe")
    return (
      <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-red-100 text-red-800 text-sm font-bold">
        🔴 Budget Breached
      </span>
    );
  return null;
...
function SavingsGoalTracker({ pacing, purchaseCost }) {
  ...
  const savingsRate =
    monthlyIncome && pacing
      ? ((monthlyIncome - pacing.totalSpent) / monthlyIncome) * 100
      : null;
  const postPurchaseRate =
    monthlyIncome && pacing && purchaseCost
      ? ((monthlyIncome - pacing.totalSpent - purchaseCost) / monthlyIncome) *
        100
      : null;
    ...
    <Card>
      ...
      {editing ? (
        ...
        <div className="text-sm text-gray-500 bg-amber-50 border border-amber-200 rounded-lg p-3">
          💡 Set your monthly income or savings target to see how each purchase
          impacts your savings rate.
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            {purchaseCost > 0 && postPurchaseRate !== null && (
              <div
                className={`rounded-xl p-3 text-center ${postPurchaseRate < 0 ? "bg-red-100" : postPurchaseRate < savingsRate - 10 ? "bg-amber-50" : "bg-green-50"}`}
              >
                <div
                  className={`text-xl font-extrabold ${postPurchaseRate < 0 ? "text-red-700" : "text-gray-700"}`}
                >
          ...
          {savingsRate !== null && (
            <div className="space-y-1.5">
              ...
              {purchaseCost > 0 &&
                postPurchaseRate !== null &&
                savingsRate > postPurchaseRate && (
                  <p className="text-xs text-red-600 font-medium">
                    ⚠️ This purchase reduces your savings rate by{" "}
                    {(savingsRate - postPurchaseRate).toFixed(1)} percentage
                    points.
                  </p>
                )}
            </div>
...
function HistoryItem({ entry, onAddExpense }) {
  ...
  return (
    <div
      className={`border rounded-xl p-4 flex items-center justify-between gap-3 ${ratingColors[entry.rating]}`}
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-500">
            {entry.category} · ${entry.cost.toFixed(2)} · {entry.time}
          </p>
        </div>
      </div>
...
/* ─────────────────────────────────────────────
   MAIN COMPONENT
───────────────────────────────────────────── */
function PacingAnalyzer({ expenses, isLoading = false }) {
  ...
  const loadData = useCallback(async () => {
    try {
      setDataLoading(true);
      const { startDate, endDate, totalDays, daysPassed } =
      ...
      const totalSpent = expenseRes.data.reduce(
  ...
  const analyze = (e) => {
    ...
    setTimeout(() => {
      ...
      if (purchaseCost > remaining) {
        rating = "unsafe";
        reasons.push(
          `This purchase ($${purchaseCost.toFixed(2)}) exceeds your remaining total budget ($${remaining.toFixed(2)}).`,
        );
      ...
      if (matchedBudget) {
        ...
        if (purchaseCost > catRemaining) {
          ...
          reasons.push(
            `This exceeds your ${selectedCategory} budget by $${overBy.toFixed(2)} ($${categorySpent.toFixed(2)} already spent of $${catBudget.toFixed(2)}).`,
          );
          ...
          const topOther = Object.entries(otherCats).sort(
            (a, b) => b[1] - a[1],
          )[0];
          if (topOther) {
            ...
            const cutPerWeek =
              weeksLeft > 0
                ? (overBy / weeksLeft).toFixed(2)
                : overBy.toFixed(2);
            suggestions.push(
              `💡 To absorb this, cut ${topOther[0]} by $${cutPerWeek}/week for the remaining ${weeksLeft} week(s).`,
            );
          }
          ...
          reasons.push(
            `✅ Within ${selectedCategory} budget. You have $${catRemaining.toFixed(2)} remaining in this category.`,
          );
        }
        ...
        if (rating === "safe" && purchaseCost > remaining * 0.5) {
          rating = "warning";
          reasons.push(
            `No budget set for ${selectedCategory}. This purchase uses ${((purchaseCost / totalBudget) * 100).toFixed(1)}% of your total monthly budget.`,
          );
          suggestions.push(
            `💡 Consider setting a monthly budget for "${selectedCategory}" to track this spending category precisely.`,
          );
        } else if (rating === "safe") {
          reasons.push(
            `No specific budget for "${selectedCategory}", but this purchase is within your overall budget capacity.`,
          );
        }
      }
      ...
      const newDailySafe =
        daysLeft > 0 ? (remaining - purchaseCost) / daysLeft : 0;
      if (newDailySafe < 0 && rating === "safe") rating = "warning";

      if (rating === "safe") {
        reasons.push(
          `Great news! This purchase fits within your remaining budget of $${remaining.toFixed(2)}.`,
        );
        suggestions.push(
          `✅ After this purchase, your safe daily spend limit becomes $${Math.max(newDailySafe, 0).toFixed(2)}/day for the remaining ${daysLeft} day(s).`,
        );
      } else if (rating === "warning") {
        suggestions.push(
          `📅 You have ${daysLeft} day(s) left this month. Your remaining daily budget after this purchase: $${Math.max(newDailySafe, 0).toFixed(2)}/day.`,
        );
      }
      ...
      if (pacing && pacing.isSpeeding && rating === "safe") {
        rating = "warning";
        reasons.push(
          `⚡ You are already spending faster than the calendar pace (${pacing.budgetProgress.toFixed(1)}% budget used vs ${pacing.calendarProgress.toFixed(1)}% of month elapsed).`,
        );
        suggestions.push(
          `💡 To get back on track, limit spending to $${pacing.dailySafeSpend.toFixed(2)}/day for the rest of the month.`,
        );
      }
  ...
  const handleAddAsExpense = async (entryOrResult) => {
    ...
    try {
      ...
      setHistory((prev) =>
        prev.map((h) =>
          h.id === entry.id ? { ...h, addedAsExpense: true } : h,
        ),
      );
      addToast(
        "Expense Added!",
        `${entry.item} ($${entry.cost.toFixed(2)}) has been recorded.`,
        "info",
      );
      // Refresh data
      await loadData();
      ...
      addToast(
        "Error",
        "Could not add expense. Please try manually.",
        "warning",
      );
    }
  ...
  const handleHistoryAddExpense = async (histEntry) => {
    try {
      ...
      setHistory((prev) =>
        prev.map((h) =>
          h.id === histEntry.id ? { ...h, addedAsExpense: true } : h,
        ),
      );
      addToast(
        "Expense Added!",
        `${histEntry.item} ($${histEntry.cost.toFixed(2)}) has been recorded.`,
        "info",
      );
      await loadData();
    } catch (err) {
  ...
  const categoryBreakdown = budgets
    ...
    .map((b) => {
      const spent = monthlyExpenses
        .filter(
          (e) => e.category.toLowerCase() === b.category_name.toLowerCase(),
        )
        .reduce((s, e) => s + Number(e.amount), 0);
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              {history.map((entry) => (
                <HistoryItem
                  key={entry.id}
                  entry={entry}
                  onAddExpense={handleHistoryAddExpense}
                />
              ))}
            </div>
          )}
      ...
      {pacing ? (
        <Card>
          ...
          {/* Status Banner */}
          <div
            className={`flex items-center gap-3 p-3 rounded-lg border mb-6 ${speedStatus?.bg}`}
          >
            <span className={`text-base font-semibold ${speedStatus?.color}`}>
              {speedStatus?.label}
            </span>
            <span className="text-sm text-gray-600">
              {pacing.daysPassed} of {pacing.totalDays} days elapsed (
              {pacing.calendarProgress.toFixed(0)}% of month)
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-6">
            ...
            <div className="flex flex-col items-center justify-center gap-1">
              <div className="text-3xl font-extrabold text-blue-600">
                ${pacing.dailySafeSpend.toFixed(2)}
              </div>
            ...
            <div className="flex flex-col items-center justify-center gap-1">
              <div
                className={`text-3xl font-extrabold ${pacing.remaining < 0 ? "text-red-600" : "text-green-600"}`}
              >
      ...
      <Card>
        ...
        <p className="text-sm text-gray-500 mb-5">
          Enter a potential purchase below. The engine instantly evaluates it
          against your live budget, pacing data, and savings goals.
        </p>
        ...
        {result && (
          ...
          >
                ...
                <div>
                  ...
                  <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min((result.categorySpent / Number(result.matchedBudget.amount)) * 100, 100)}%`,
                      }}
                    />
                    <div
                  ...
                  <div className="flex justify-between text-[10px] text-gray-400 mt-0.5">
                    ...
                    <span>
                      Limit: ${Number(result.matchedBudget.amount).toFixed(0)}
                    </span>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(((pacing?.totalSpent || 0) / result.totalBudget) * 100, 100)}%`,
                      }}
                    />
                    <div
                  ...
                  <div className="flex justify-between text-[10px] text-gray-400 mt-0.5">
                    <span>
                      Current: ${(pacing?.totalSpent || 0).toFixed(0)}
                    </span>
            ...
            <div
              className={`text-sm font-semibold ${result.newDailySafe > 0 ? "text-gray-700" : "text-red-700"}`}
            >
              📆 New Safe Daily Spend after this purchase: $
              {result.newDailySafe.toFixed(2)}/day for {result.daysLeft}{" "}
              remaining day(s)
            </div>
          ...
          <div className="flex items-center gap-3 p-4 bg-amber-50 rounded-lg border border-amber-200">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                To unlock the full power of the Decision Engine, go to the{" "}
                <strong>Budgets</strong> tab and set monthly limits per
                category. The engine will then give you precise
                Safe/Warning/Unsafe ratings.
              </p>
            </div>
          </div>
        </Card>
      ...
      {categoryBreakdown.length > 0 && pacing && pacing.totalBudget > 0 && (
        <Card>
          <h3 className="text-lg font-semibold text-gray-700 mb-2">
            🎯 Spending Impact Analyzer
          </h3>
          <p className="text-sm text-gray-500 mb-4">
            See how cutting each category's spending would free up daily budget
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {categoryBreakdown.map((cat, i) => {
                  ...
                  const dailyFreed =
                    pacing.daysLeft > 0 ? cut50 / pacing.daysLeft : 0;
                  return (
                    <tr
                      key={cat.name}
